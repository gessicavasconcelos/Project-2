...
loadPkg("gplots")
levels(df1$SEX) <- c("Unknown", "Male", "Female")
gendercontable = xtabs( ~ CATEGORY + SEX, data = df1)
chisqsex = chisq.test(gendercontable)
chisqsex
xkabledply(gendercontable, "Contingency Table: Gender vs. Arrest ")
```

## Building a Multinomial Model 
```{r Logit, results='show'}

logit1 <- glm(Category ~ Hour + Age + Defendant_PSA + Race + Sex + Offense_PSA + Offense_District, data = Features, binomial(link = "logit"))

table <- summary(logit1)
exp(coef(logit1))
xkabledply(table)
...
```{r AUC, results ='show'}
loadPkg("pROC") 
Features$prob=predict(logit1, type = c("response")) 
h <- roc(Category~prob, data=Features)
auc(h) 
plot(h)
```
