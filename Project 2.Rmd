...
gwu_crimes <- subset(df, CATEGORY != 1:2 & CATEGORY != 4:13 & CATEGORY !=15:19  & CATEGORY != 21:25 & CATEGORY != 27:29)


# rank violent crimes
gwu_crimes$CATEGORY <- factor(violent_crimes$CATEGORY, levels = c(3, 14, 20,26))
dcmap1 <- qmap('gwu', zoom = 14, color = 'color')

dcmap1 +geom_point(aes(x = OFFENSE_LONGITUDE, y = OFFENSE_LATITUDE, size = CATEGORY,colour = CATEGORY), data = violent_crimes )+ theme(legend.box = "vertical",legend.position = "topleft")
...
```{r When do the most violent crimes happen?}
...
newdata <- subset(df, CATEGORY = 26, select=c(YEAR, CATEGORY))
str(newdata)
hist(newdata)
```
